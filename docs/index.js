...
import * as Storymatic from "https://esm.sh/storymatic@2.0.79";
...
function jsx(tag, attrs, ...children) {
    if (typeof tag === "string") {
        ...
        for (let attr in attrs) {
            if (attr === "style") {
                ...
                el.addEventListener(attr.slice(2, 3).toUpperCase() + attr.slice(3), attrs[attr]);
            }
            else {
                el[attr] = attrs[attr];
            }
    ...
    try {
        // @ts-ignore
        return new tag(children.length ? Object.assign(Object.assign({}, attrs), { children }) : attrs);
    }
    catch (_a) {
        // @ts-ignore
        return tag(children.length ? Object.assign(Object.assign({}, attrs), { children }) : attrs);
    }
}
function addError(href) {
    let els = document.querySelectorAll(`a[href="${href}"]`);
    if (els[0].textContent.includes("(error)")) {
        els[0].textContent = els[0].textContent.replace("(error)", "(2 errors)");
        els[1].children[0].textContent = els[1].children[0].textContent.replace("(error)", "(2 errors)");
    }
    else if (els[0].textContent.includes("errors)")) {
        els[0].textContent = els[0].textContent.replace(/\d+ errors/, (match) => `${+match.split(" ")[0] + 1} errors`);
        els[1].children[0].textContent = els[1].children[0].textContent.replace(/\d+ errors/, (match) => `${+match.split(" ")[0] + 1} errors`);
    }
    else {
...
function compile(text, flags = { typescript: true }, throws = false) {
    try {
...
(function (CompileOnUpdate) {
    CompileOnUpdate.extension = EditorView.updateListener.of((update) => {
        ...
        if (!secondaryWindow) {
            throw new Error("Extension `CompileOnUpdate` requires a `CompileOnUpdate.SecondaryWindow` facet.");
        }
        secondaryWindow.dispatch(secondaryWindow.state.update({
            changes: {
                from: 0,
    ...
    CompileOnUpdate.SecondaryWindow = Facet.define({
        combine(value) {
            return value[0];
        },
    });
    CompileOnUpdate.CompileOptions = Facet.define({
        combine(value) {
            return value.reduce((prev, next) => (Object.assign(Object.assign({}, next), prev)), {
                typescript: true,
            });
        },
    });
})(CompileOnUpdate || (CompileOnUpdate = {}));
window.$docsify = {
            ...
            hook.doneEach(async () => {
                ...
                for (let el of codes) {
                    ...
                    try {
                        ...
                        if (el)
                            addError(el.children[0].getAttribute("href"));
                    }
                    jsb.addEventListener("click", () => {
                        ...
                        window.console = Object.assign(Object.assign({}, console), { assert(condition, ...data) {
                                if (!condition) {
                                    if (data.length) {
                        ...
                        let result = __eval(compile(editor.state.doc.sliceString(0), { typescript: false }));
                        if (result !== undefined) {
